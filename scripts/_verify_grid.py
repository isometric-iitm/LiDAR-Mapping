...
print(f"rendered cells: {len(snap['rows'])}, frame={snap['frame']}")
...
for k in range(60):
    ...
    d = grid2.compute_delta()
    sizes.append(len(d["rows"]) + len(d["freed"]))
dt = time.time() - t0
...
snap2 = grid2.compute_snapshot()
print(f"snapshot rendered cells: {len(snap2['rows'])}")
print("sample cell:", snap2["rows"][0].tolist(), "cls", int(snap2["cls"][0]))